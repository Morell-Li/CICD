/**
 * OpenAPI V3 合约（linear）止盈止损全链路：设模式 → 市价开仓 → 设 TP/SL → 校验 → 清除
 *
 * 用例文档：qa-design/cases/openapi-v3/order-lifecycle.md（TC-3140）
 */
...
import {
  ...
  ORDER_SYMBOL,
  ORDER_SETTLE_COIN,
  buildClearTradingStopBody,
  buildCreateMarketOrderBody,
  buildSetTpslModeBody,
  buildTradingStopBody,
  findPositionRow,
  pickMarkPrice,
  pickPositionSide,
  pickPositionSize,
...
const SPEC_NAME = 'order-lifecycle-linear-tpsl';
...
function flowAnno(step: string): Parameters<typeof test>[1] {
  return {
    ...
    annotation: [
      ...apiCaseAnnotations(`TC-3140 | 止盈止损串行 - ${step}`),
...
test.describe.serial('order-lifecycle-linear-tpsl｜合约止盈止损：设模式 → 开仓 → 设 TP/SL → 校验 → 清除', () => {
  test.beforeAll(() => {
  ...
  test.afterAll(async ({ request }) => {
    if (await isOpenApiLive(request)) {
      await signedPost(request, PATH_POSITION_TRADING_STOP, buildClearTradingStopBody());
      await signedPost(request, PATH_ORDER_CREATE, buildCreateMarketOrderBody({
        side: 'Sell',
        reduceOnly: true,
        orderLinkId: `qa-tpsl-close-v3-${Date.now()}`,
      }));
  ...
  test('步骤 3/5：设置止盈止损 trading-stop', flowAnno('设止盈止损'), async ({ request }, testInfo) => {
    ...
    const stopBody = buildTradingStopBody(markPrice, side);
    const { body } = await exchangePost(
      testInfo,
      '3-trading-stop',
      request,
